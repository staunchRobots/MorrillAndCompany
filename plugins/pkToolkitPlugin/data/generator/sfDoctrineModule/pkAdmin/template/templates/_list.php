<div class="pk-admin-list">
  [?php if (!$pager->getNbResults()): ?]
    <p>[?php echo __('No result', array(), 'pk-admin') ?]</p>
  [?php else: ?]
    <table cellspacing="0" class="pk-admin-list-table">
      <thead>
        <tr>
					<?php if ($this->configuration->getValue('list.batch_actions')): ?>
          	<th id="pk-admin-list-batch-actions"><input id="pk-admin-list-batch-checkbox-toggle" class="pk-admin-list-batch-checkbox-toggle pk-checkbox" type="checkbox"/></th>
					<?php endif; ?>
          	[?php include_partial('<?php echo $this->getModuleName() ?>/list_th_<?php echo $this->configuration->getValue('list.layout') ?>', array('sort' => $sort)) ?]
					<?php if ($this->configuration->getValue('list.object_actions')): ?>
          	<th id="pk-admin-list-th-actions">[?php echo __('Actions', array(), 'pk-admin') ?]</th>
					<?php endif; ?>
        </tr>
      </thead>
      <tfoot>
        <tr>
          <th colspan="<?php echo count($this->configuration->getValue('list.display')) + ($this->configuration->getValue('list.object_actions') ? 1 : 0) + ($this->configuration->getValue('list.batch_actions') ? 1 : 0) ?>">
						<div class="pk-admin-list-results">
	            [?php echo format_number_choice('[0] no result|[1] 1 result|(1,+Inf] %1% results', array('%1%' => $pager->getNbResults()), $pager->getNbResults(), 'pk-admin') ?]
	            [?php if ($pager->haveToPaginate()): ?]
	              [?php // echo __('(page %%page%%/%%nb_pages%%)', array('%%page%%' => $pager->getPage(), '%%nb_pages%%' => $pager->getLastPage()), 'pk-admin') ?]
	            [?php endif; ?]
						</div>
            [?php if ($pager->haveToPaginate()): ?]
      ...
      <tbody>
        [?php $n=1; $total = count($pager->getResults()); foreach ($pager->getResults() as $i => $<?php echo $this->getSingularName() ?>): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?]
          <tr class="pk-admin-row [?php echo $odd ?] [?php echo ($n == $total)? 'last':'' ?]">
						<?php if ($this->configuration->getValue('list.batch_actions')): ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#pk-admin-list-batch-checkbox-toggle').click(function(){
			$('.pk-admin-batch-checkbox').each( function() {
